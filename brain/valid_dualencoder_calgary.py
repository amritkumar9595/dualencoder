...
def run_model(args, model, data_loader):
    ...
    with torch.no_grad():
        ...
        for (iter,data) in enumerate(tqdm(data_loader)):
            ...
            # print("recons,target",recons.squeeze(0).shape,target.squeeze(0).shape)
            
            # metrics = Metrics(METRIC_FUNCS)
            # metrics.push(target.float().squeeze(0).numpy(), recons.float().squeeze(0).numpy())
...
def main(args):
    ...
    # metrics = run_model(args, model, data_loader)
    ...
    print("check reconstructions @ :",args.out_dir)
